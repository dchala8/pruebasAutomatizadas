...
(async () => {
    ...
    await page.screenshot({ path: caseFolder + '10-Old-tag.jpg' })
    tagname = tagName + "Modified"
    await page.type('input[name="name"]', "Modified");
    await page.click('.gh-btn-primary').catch(() => console.log("error in click on Save button")) //save button
    await page.screenshot({ path: caseFolder + '11-New-tag-updated.jpg' })
    ...
    await page2.screenshot({ path: caseFolder + '12-Tag-in-post-updated.jpg' })
    if (toTagsmain2) {
        console.log("Yes, it was")
    }else{
        console.log("No, it was not")
