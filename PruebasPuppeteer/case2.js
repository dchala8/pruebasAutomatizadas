...
(async()=>{
    ...
    await new Promise(r => setTimeout(r, 1500));
    await page.screenshot({path: './case2/logged.png'})
    await new Promise(r => setTimeout(r, 1500));

    //Ingresar a Posts
    await page.click("#ember16")
    await page.screenshot({path: './case2/posts.png'})
    await new Promise(r => setTimeout(r, 1500));

    //Ingresar a Crear posts
    await page.click(".ember-view.gh-btn.gh-btn-primary.view-actions-top-row")
    await page.screenshot({path: './case2/newPost.png'})
    await new Promise(r => setTimeout(r, 1500));

    //Crear Nuevo Post
    await page.type(".gh-editor-title.ember-text-area.gh-input.ember-view", "caso2")
    await page.screenshot({path: './case2/setTituloPost.png'})
    await new Promise(r => setTimeout(r, 2500));
    await page.type(".koenig-editor__editor.__mobiledoc-editor.__has-no-content","caso2")
    await new Promise(r => setTimeout(r, 1500));
    await page.click(".ember-view.ember-basic-dropdown-trigger")
    await new Promise(r => setTimeout(r, 1500));
    await page.screenshot({path: './case2/abrirPublish.png'})
    ...
    await new Promise(r => setTimeout(r, 1500));
    await page.screenshot({path: './case2/published.png'})
    ...
    const grabItems = await page.evaluate(() => {
        ...
        elements.forEach((publication)=> {
            ...
            if(title.innerHTML.includes("caso2")){
    ...
    const [button] = await page.$x("//a[contains(., 'caso2')]");
    if (button) {
        await button.click();
    }
    await new Promise(r => setTimeout(r, 1500));
    await page.screenshot({path: './case2/intoPost.png'})

    //Elimino el post
    await new Promise(r => setTimeout(r, 1500));
    await page.click(".settings-menu-toggle.gh-btn.gh-btn-editor.gh-btn-icon.icon-only.gh-btn-action-icon")
    await new Promise(r => setTimeout(r, 1500));
    await page.screenshot({path: './case2/openMenu.png'})
    await page.click(".gh-btn.gh-btn-hover-red.gh-btn-icon.settings-menu-delete-button")
    await new Promise(r => setTimeout(r, 1500));
    await page.screenshot({path: './case2/clickDelete.png'})
