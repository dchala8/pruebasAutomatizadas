...
const userEmail = Cypress.config('email')
const userPassword = Cypress.config('password')
...
function newScrollTo(x,y){
    // cy.get('.gh-main').scrollTo(curX, curY)
...
describe( `${appName} under monkeys`, function() {
    const {username, password} = {username:userEmail, password:userPassword}
    //Listener
    ...
    Cypress.on('uncaught:exception', (err, runnable) => {
        cy.task('genericLog', {'message':`The test failed with the following error: ${err}`});
        cy.task('genericReport', {'html': `<h1><strong>Test failed with the error: </strong>${err}</h1>`});
        return false;
    });
