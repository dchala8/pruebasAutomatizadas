#  Instalaicion y configuracion del Cypress Random Tester (Monkey)
...
dirigirse al archivo monkey-config.json
cambiar la baseUrl por aquella que este usando la instalacion de ghost en  pruebas
cambiar las variables de usuario y contraseña, usar aquellas que se encunetren en uso para el usuario admin de la version de ghost a probar

# Ejecucion
Para ejecutar use 

cypress run -C monkey-config.json  --headed
o
cypress run -C smart-monkey-config.json  --headed
